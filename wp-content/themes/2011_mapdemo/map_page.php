...
<script>
	...
	//Validation--Functions which support front-end validation of user input
	...
	$("#add_food_exchange_form").validate({
	    debug: true, 
            rules: {
                ...
                foodexchangedate: { required: true , date: true },
	...
	$("#add_work_exchange_form").validate({
		...
		workexchangedate: { required: true , date: true },
          	expertise: { requied: true }
		}
		});
...
<div id="primary">
	...
	$("#add_garden").click(function () {
	  ...
	  var userid = <?php echo $current_user->ID ?>;
          ...
          var gardenname = $("input[id=garden_name]").val();
          var address = $("input[id=street_address]").val();
          var city = $("input[id=city]").val();
          var state = $("input[id=state]").val();
          var zipcode = $("input[id=zip_code]").val();
          var squarefootage = $("select[id=square_footage]").val();
          var gardendescription = $("textarea[id=description]").val();
	  var geocode_address = address + "  " + city + "  " +  state;
	  //calling the google geocoding service
	  var geocoder = new google.maps.Geocoder();
		...
		console.log(results);
           	...
           	console.log(lat,lng);
		returnLatLng(lat, lng);
		}
		else { alert("Google Maps had some trouble finding" + status);}
		});
	  function returnLatLng (value1, value2){
	  	latitude = value1;
		longitude = value2;
		console.log(latitude, longitude);
	  	//serializing the data for post
          	var gardendata = 'userid=' + userid + '&gardenname=' + gardenname + '&address=' + address + '&city=' + city + '&state=' + state + '&zipcode=' + zipcode + '&squarefootage=' + squarefootage + '&gardendescription=' + gardendescription + '&latitude=' + latitude + '&longitude=' + longitude;
		console.log(gardendata);
         	...
         	var writegarden = $.ajax({
          	...
          	// callback handler that will be called on ajax success
          	writegarden.done(function (response, textStatus, jqXHR){
          	// alert user based upon the error code returned from mysql
          	if (response == 1062){ 
                	alert("You can only enter one garden per user. Sorry");}
         	});
          	// callback handler that will be called on ajax failure
	...
	<!-- Add_Food_Exchange_Form --> 
        <div id="food_exchange_dialog" class="dialog" title="Add Food Exchange">
            <div class="food_exchange_form_container">
                ...
                <div class="element">
                        <label for="exchange_quantity">Quantity for Exchange</label>
                        <select id="exchange_quantity" name="exchange_quantity" class="required" value="" ></select>
		</div>
                <div class="element">
                        <label for="foodexchangedate">Avail Date</label>
                        <input id="foodexchangedate" name="foodexchangedate" class="required" value="" />
                </div>
                <div class="element">
                        <label for="desired_exchange">Desired Exchange</label>
	...
	<!-- Add_Work_Exchange_Form --> 
        <div id="work_exchange_dialog" class="dialog" title="Add Work Exchange">
            <div class="work_exchange_form_container">
                <form id="add_work_exchange_form" name="add_work_exchange_form" method="post" action="">
                <div class="element">
                ...
                <div class="element">
                        <label for="workexchangedate">Avail Date</label>
                        <input id="workexchangedate" name="workexchangedate" class="required" value="" />
                </div>
		<div class="element">
	...
	$("#add_work_exchange").click(function () {
	  ...
	  dataType: "json",
          ...
          // alert user based upon the error code returned from mysql
          if (response == 1062){ 
                alert("You can only enter one work exchange per user. Sorry");}
          });
          // callback handler that will be called on failure
	...
	echo "  <select id=\"plant_removal\" class=\"text\" >\n";
	...
	?>
        ...
        $("#remove_food_exchange").click(function (event) {
          ...
          //accessing php glogal variable ID which is the primary key of 
          var userid = <?php echo $current_user->ID ?>;
	  var exchangeplant = $("select[id=plant_removal]");
	  ...
	  removefood ()
		...
		.done(function (response, textStatus, jqXHR){
			...
			// log a message to the console
          		console.log("Hooray, it worked!");
			} else { console.log ("Boo, it didn't work"); }
		})
	...
	<!-- Remove_Work_Exchange_Form --> 
        <div id="remove_work_exchange_dialog" class="dialog" title="Remove Work Exchange">
            <div class="form">
                ...
                <div class="element">
                        <label for="user_name">User Name</label>
                        <input type="text" id="user_name" name="user_name" class="required" value= "<?php echo $current_user->user_login ?>" readonly=true />
                </div>
                <div class="button_element">
...
<!-- Buttons -->
	...
	if (is_user_logged_in()){
		require_once(ABSPATH. '/wp-content/themes/2011_mapdemo/phphandlers/dbconnect.php');
        	...
        	//Build the SQL query
        	$query = "DELETE FROM food_exchanges WHERE available_date < DATE_SUB(NOW(), INTERVAL 2 WEEK);";
        	$query .= "SELECT plantforexchange, quantity, available_date, desired_exchange, user_nicename, user_email FROM food_exchanges, wp_users WHERE userid=ID ORDER BY available_date ASC LIMIT 10";
        	...
        	//Execute queries and store result
			if (mysqli_multi_query($conn, $query)) {
    				do {
        			/* store first result set */
        			if ($result = mysqli_store_result($conn)) {
            			while ($row = mysqli_fetch_row($result)) {
               			printf ("Plant for Exchange: %s, Quantity: %s, Available Date: %s, Desired Plant: %s , POC: %s, Email: %s \n", $row[0], $row[1], $row[2], $row[3], $row[4], $row[5]);
		...
		require_once(ABSPATH. '/wp-content/themes/2011_mapdemo/phphandlers/dbconnect.php');
                ...
                //Build the SQL query
                $query = "DELETE FROM food_exchanges WHERE available_date < DATE_SUB(NOW(), INTERVAL 2 WEEK);";
                $query .= "SELECT plantforexchange, quantity, available_date, desired_exchange FROM food_exchanges ORDER BY available_date ASC LIMIT 10";
                echo "<div id=\"exchange_display\" title=\"Food Exchange Display\">\n";
                echo    "<form id=\"food_exchange_display\">\n";
                echo       "<textarea id=\"work_exchange_text\" readonly=true >\n";
                //Execute queries and store result
                        if (mysqli_multi_query($conn, $query)) {
                                ...
                                if ($result = mysqli_store_result($conn)) {
                                while ($row = mysqli_fetch_row($result)) {
                                printf ("Plant for Exchange: %s, Quantity: %s, Available Date: %s, Desired Plant: %s  \n", $row[0], $row[1], $row[2], $row[3]);
                                } mysqli_free_result($result);}
                                } while (mysqli_next_result($conn));
                                }
                                echo "</textarea>\n";
	...
	if (is_user_logged_in()){
		...
		require_once(ABSPATH. '/wp-content/themes/2011_mapdemo/phphandlers/dbconnect.php');
                ...
                //Build the SQL query
                $query = "SELECT first_name, experience, availability, available_date, expertise FROM work_exchanges LIMIT 10;";
                //Execute query and store result
                $result = $conn->query($query);
                ...
                //Accessing resultset array
                while ($row = $result->fetch_array(MYSQLI_NUM)){
                        printf ("First Name: %s, Experience(yrs): %s, Availability (Hrs/wk): %s, Available Date: %s, Expertise: %s\n", $row[0], $row[1], $row[2], $row[3], $row[4]);
		}
                echo "</textarea>\n";
                echo    "</form>\n";
                echo "</div>\n";
